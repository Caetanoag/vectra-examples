...
class CelestialBody {
	...
	public drawTrail(renderer: CanvasRenderer) {
		for (let i = 1; i < this.trail.length; i++) {
			const t = i / this.trail.length;
			renderer.drawLine(
				this.trail[i - 1] as Vector2,
				this.trail[i] as Vector2,
				this.color.withAlpha(t * 0.6),
				1,
			);
		}
	}
...
const getGravitationalForce = (
	...
	const forceMag =
		(G * firstBody.mass * secondBody.mass) / (distance * distance);
...
const resolveCollision = (
	...
	if (!firstBody.alive || !secondBody.alive) return;

	if (!firstBody.circle.intersects(secondBody.circle)) return;
	...
	first.position = first.position.add(
		normal.scale(overlap * (second.mass / totalMass)),
	);
	second.position = second.position.subtract(
		normal.scale(overlap * (first.mass / totalMass)),
	);
	...
	second.velocity = second.velocity.subtract(
		normal.scale(impulse / second.mass),
	);
};
...
const minZoom = 0.3;
const maxZoom = 3;
let lastMouse: Vector2 | null = null;

const zoomAt = (anchor: Vector2, nextZoom: number) => {
	const clamped = Math.min(maxZoom, Math.max(minZoom, nextZoom));
	const world = center.add(
		anchor
			.subtract(center)
			.subtract(pan)
			.scale(1 / zoom),
...
const planets = [
	...
	return new CelestialBody(
		center.add(Vector2.fromPolar(angle, radius)),
		Vector2.fromAngle(angle + Math.PI / 2).scale(orbitalSpeed),
		mass,
		size,
		color,
		name,
...
let followTarget: CelestialBody | null = null;

const menu = document.querySelector("#planetMenu") as HTMLDivElement;

const menuToggle = document.querySelector("#menuToggle") as HTMLButtonElement;
...
type MenuEntry = { body: CelestialBody; label: HTMLDivElement };
const menuEntries: MenuEntry[] = bodies.map((body) => {
...
const loop = () => {
	...
	let panX = 0;
	let panY = 0;
	if (input.isKeyDown("ArrowLeft") || input.isKeyDown("a")) panX += panSpeed;
	if (input.isKeyDown("ArrowRight") || input.isKeyDown("d")) panX -= panSpeed;
	if (input.isKeyDown("ArrowUp") || input.isKeyDown("w")) panY += panSpeed;
	if (input.isKeyDown("ArrowDown") || input.isKeyDown("s")) panY -= panSpeed;
	if (panX !== 0 || panY !== 0) {
		followTarget = null;
	...
	for (let s = 0; s < SUBSTEPS; s++) {
		for (let i = 0; i < bodies.length; i++) {
			for (let j = i + 1; j < bodies.length; j++) {
				const firstBody = bodies[i] as CelestialBody;
				const secondBody = bodies[j] as CelestialBody;
		...
		for (let i = 0; i < bodies.length; i++) {
			for (let j = i + 1; j < bodies.length; j++) {
				resolveCollision(
					bodies[i] as CelestialBody,
					bodies[j] as CelestialBody,
				);
			}
		}

		bodies = bodies.filter((body) => body.alive);
		if (followTarget && !followTarget.alive) followTarget = null;
		for (let k = menuEntries.length - 1; k >= 0; k--) {
			const entry = menuEntries[k] as {
				body: CelestialBody;
	...
	starField.forEach((s) => {
		renderer.fillCircle(
			s.position,
			s.size,
			Color.fromRgb(255, 255, 255, 60 + ((s.size * 10) % 100)),
		);
	});
